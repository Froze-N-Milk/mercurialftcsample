...
@TeleOp(name = "Translational Y Acceleration Tuner")
public class TranslationalYAccelerationTuner extends OpModeEX {
	...
	public void registerTriggers() {
		new Trigger(() -> Math.abs(velocity - mecanumDriveBase.getMotionConstants().getMaxTranslationalYVelocity()) < 0.02 * mecanumDriveBase.getMotionConstants().getMaxTranslationalYVelocity())
				.setCommand(
						new LambdaCommand()
								.init(() -> {
	...
	public void loopEX() {
		...
		telemetry.addData("max velocity", mecanumDriveBase.getMotionConstants().getMaxTranslationalYVelocity());
		...
		telemetry.addData("velocity delta", velocity - mecanumDriveBase.getMotionConstants().getMaxTranslationalYVelocity());
